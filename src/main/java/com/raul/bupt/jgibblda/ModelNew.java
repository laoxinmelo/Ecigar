...
public class ModelNew {
	...
	public static String twordsSuffixnew;
	//---------------------------------------------------------------
	...
	public boolean saveModelTheta(String filename){
		try{
			...
			for (int i = 0; i < M; i++){
				for (int j = 0; j < K; j++){
					writer.write(df.format(theta[i][j]) + " ");
				}
				writer.write("\r\n");
			}
			writer.close();
	...
	public boolean saveModelPhi(String filename){
		try {
			...
			for (int i = 0; i < K; i++){
				for (int j = 0; j < V; j++){
					writer.write(df.format(phi[i][j]) + " ");
				}
				writer.write("\r\n");
			}
			writer.close();
	...
	public boolean saveModel(String modelName)throws IOException{
		...
		if (twords > 0){
			if (saveModelTwords(dir + File.separator + modelName + twordsSuffix))
			{
				...
				tio.changeTWordFile(dir + File.separator + modelName + twordsSuffix, dir + File.separator + modelName + twordsSuffixnew);
	 ...
	 */
	public boolean initNewModel(String modelDir, LDADataset newData, ModelNew trnModel){
		//		if (!init(option))
		//			return false;
...
//		System.out.println("V:" + V);
	 ...
	 */
	public boolean initEstimatedModel(String modelDir){
		//		if (!init(option))
		//			return false;
